...
pub fn disc_sample (
    ...
    while active.len() > 0 {
        ...
        let new_points = sample_poisson_points(30, size, width, height, min_offset, &point, &mut grid, gen);
...
fn add_borders (mut grid: Vec<Vec<[f64; 2]>>, mut active: Vec<[f64; 2]>, mut points: Vec<f64>, size: f64, cols: usize, rows: usize, width: f64, height: f64) -> (Vec<Vec<[f64; 2]>>, Vec<[f64; 2]>, Vec<f64>)  {
    let size = size / 2.0;
    let offset = 5e-2;
    let cx = width / 2.0;
    let cy = height / 2.0;
